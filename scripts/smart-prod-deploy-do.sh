...
# Smart Deploy - Digital Ocean prod
# Robust, idempotent deployment script for Digital Ocean prod environment
...
# Configuration
ENVIRONMENT="prod"
CLUSTER_NAME="${CLUSTER_NAME:-listapro-prod-cluster}"
REGISTRY_NAME="${REGISTRY_NAME:-listapro-prod-registry}"
...
echo -e "${BLUE}🌊 Digital Ocean Smart Deploy - prod${NC}"
echo "=============================================="
...
setup_cluster_infrastructure() {
    ...
    # Label namespace
    kubectl label namespace "$NAMESPACE" environment=prod --overwrite
...
main() {
    ...
    echo "🎯 Starting Digital Ocean prod deployment..."
    echo "Action: $action"
    ...
    case "$action" in
        ...
        "apply")
            ...
            ;;
        "destroy")
            echo -e "${RED}🗑️  Destroying prod infrastructure...${NC}"
            if [[ "$SKIP_CONFIRM" != "1" ]]; then
                read -p "Are you sure you want to destroy the prod infrastructure? (yes/no): " confirm
                if [[ "$confirm" != "yes" ]]; then
